...
const ONNX_BASE = new URL("./", import.meta.url).href;
...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    ...
    const ortModule = await import("./ort.wasm.esm.min.js");
    ...
    if (ort.env) {
      ...
      if (ort.env.wasm) {
        ort.env.wasm.numThreads = 1; /* single-thread: avoid COOP/COEP / SharedArrayBuffer */
